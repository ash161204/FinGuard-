...
export function buildTaxWizardReport(documentResults = []) {
  ...
  const bestTax = isNewBetter ? totalTaxNew : totalTaxOld;
  const missedDeductions = missedItems.slice(0, 8);
  const investmentSuggestions = filteredInvestments.slice(0, 6);
  ...
  return {
    ...
    deductions80C: d80C,
    nps80CCD1B: nps1b,
    elssAmount: inputs.elss,
    taxDeducted: inputs.taxDeducted,
    refundOrPayable: inputs.taxDeducted ? inputs.taxDeducted - bestTax : 0,
...
export function buildMfXRayReport(documentResults = [], options = {}) {
  ...
  return {
    ...
    overlaps: overlaps.slice(0, 8).map((item) => ({
      ...item,
      similarity: item.overlap / 100,
    })),
    ...
    // Canonical allocation contract for score consumers
    allocationPct: {
      equity: equityPct * 100,
      debt: debtPct * 100,
      gold: goldPct * 100,
    },
  };
 ...
 * @param {number}  monthlyIncomeRaw – gross monthly income
...
export function buildMoneyHealthScore(
  ...
  if (mfReport) {
    const normalizeAllocationPct = (value) => {
      const numeric = Number(value || 0);
      return numeric <= 1 ? numeric * 100 : numeric;
    };
    const alloc = mfReport.allocationPct || {
      equity: mfReport.equityPct ?? 0,
      debt: mfReport.debtPct ?? 0,
      gold: mfReport.goldPct ?? 0,
    };
    const eqPct = normalizeAllocationPct(alloc.equity);
    const debtPct = normalizeAllocationPct(alloc.debt);
    const goldPct = normalizeAllocationPct(alloc.gold);
    ...
    if (mfReport.overlaps && mfReport.overlaps.length > 0) {
      const highOverlap = mfReport.overlaps.filter((o) => {
        const similarity = o.similarity ?? ((o.overlap ?? 0) / 100);
  ...
  if (taxReport) {
    const parts = [];
    let pts = 0;
    const sec80C = taxReport.deductions80C ?? taxReport.oldRegime?.deductionBreakdown?.d80C ?? 0;
    const bestRegime = taxReport.bestRegime || taxReport.recommendedRegime || taxReport.recommendation || "";
    const missedDeductions = taxReport.missedDeductions || taxReport.missedItems || [];
    const nps80CCD1B = taxReport.nps80CCD1B ?? taxReport.oldRegime?.deductionBreakdown?.nps1b ?? 0;
    const elssAmount = taxReport.elssAmount ?? 0;
    const actualTax =
      taxReport.bestTax
      ?? taxReport.taxPayable
      ?? (bestRegime === "New" ? taxReport.newRegime?.totalTax : taxReport.oldRegime?.totalTax)
      ?? 0;
    const grossIncome = taxReport.grossIncome || taxReport.oldRegime?.grossSalary || annualIncome;
    ...
    const missedCount = missedDeductions.length;
    if (missedCount > 0) {
      pts -= missedCount * 5;
